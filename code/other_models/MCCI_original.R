...
# https://github.com/mxjki/Matrix_Completion_For_Complex_Survey_with_Multivariate_Missingness/blob/655457265ad510cef6ccf9d458402437275f1777/Single-stage%20sampling/SMC_functions.R
########################## Validation error
cv_validate_sim <- function(
  diagD, tau2_grid, tau1_grid_length, svdY, alpha, uvalidatemat,
  Xbeta_validate, umat_validate
    ...
    for (i in 1:length(tau2_grid)) {
        B_validate <- as.vector(sqrt(diagD) * SVTE_alpha(
            svdY$u, svdY$d, svdY$v, tau2_grid[i],
            alpha
        ))[uvalidatemat != 0]
...
######################################## Proposed Method
SMCfit_cv <- function(
  Y, X.cov, prob.mis, pps.prob, sample.size, design, nfolds = 5,
  tau1_grid = seq(0, 1, length = 30), tau2_grid = seq(0.9, 0.1, length = 30), alpha_grid = seq(0.992,
    ...
    index.s <- sample.gen(
        N.popu = N.popu, pps.prob = pps.prob, sample.size = sample.size,
        design = design
    )
    ...
    missing.index$group <- sample(
        x = 1:5, size = nrow(missing.index), replace = TRUE,
        prob = rep(1, 5)
    )
    ...
    for (fold_num in 1:nfolds) {
        ...
        Xbetavalidate <- append(Xbetavalidate, list(matrix(
            nrow = length(tau1_grid),
    ...
    for (fold_num in 1:nfolds) {
        for (alpha_num in 1:length(alpha_grid)) {
            cv[fold_num, , , alpha_num] <- cv_validate_sim(
                diagD, tau2_grid, length(tau1_grid),
    ...
    return(list(
        cv = cv, tau_beta_ratio = tau1_grid[cv_grid[2]], tau_svd_ratio = tau2_grid[cv_grid[1]],
...
SMCfit <- function(Aobs, X, diagD, tau_beta_ratio, tau_svd_ratio, alpha_ratio) {
    ...
    Bhat <- SVTE_alpha(
        svdPXpAobsw$u, svdPXpAobsw$d, svdPXpAobsw$v, tau_svd_ratio,
        alpha_ratio
    )

    rankB <- sum(pmax(
        svdPXpAobsw$d - svdPXpAobsw$d[1] * tau_svd_ratio * alpha_ratio,
        0
    ) > 0)
...
var.double.robust <- function(
  fitted.sample, ori.sample, est.p, pps.prob, N.popu,
  sample.size
) {
...
one.iter <- function(
  seed_num, Y, X.cov, n.low.rank, prob.mis, pps.prob, sample.size,
  design, cv_ratio_SMC
) {
    ...
    for (ques.i in 1:ncol(sample.mis.result)) {
        ...
        if (design == "SRS") {
            ...
            est.i.hat <- glm(y.i.hat ~ x.sample[, 1:3],
                weights = weight.i.hat / 1000,
    ...
    sample.mis.SMC <- SMCfit(
        Aobs = sample.mis.result, X = x.sample, diagD, cv_ratio_SMC$tau_beta_ratio,
    ...
    for (i in 1:(ncol(Y))) {
        est.result[i] <- est.double.r(fitted.sample = fitted.sample.SMC[, i], ori.sample = sample.mis.result[
            ,
            i
        ], est.p = Phat[, i], pps.prob = x.prob, N.popu = N.popu, sample.size = sample.size) -
            target[i]

        var.result[i] <- var.double.robust(
            fitted.sample = fitted.sample.SMC[, i],
