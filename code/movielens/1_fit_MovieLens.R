...
imrxz_cv <- IMR::imr_tune(model_data, grid, final_fit = FALSE,
                                          fast_nuclear = FALSE,
...
model_data <- update(model_data,
                     ...
                     col_covariates = FALSE); model_data
...
imri_cv <- IMR::imr_tune(model_data, grid, final_fit = FALSE,
                          fast_nuclear = FALSE,
                          nuclear_log_scale = FALSE,
                          convergence=convergence, n_cores=7,
                          seed = seed, verbose=1)
...
start_si <- Sys.time()
simpute_cv <- simpute.cv(y_full = model_data$Y,
...
M <- MCAI.fit(Y, data$X, save_to_file = T,
